...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] = textFile.map(WikipediaData.parse)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    // return all languages mentioned in a given article
    def langsMentionedInArticle(article: WikipediaArticle): List[String] = {
      langs.filter(article.mentionsLanguage)
    }

    rdd
      .flatMap(article => langsMentionedInArticle(article))
      .map(lang => (lang, 1))
      .reduceByKey(_ + _)
      .collect
      .sortBy(_._2)
