...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    val questionAnswers: RDD[(Question, Iterable[Answer])] =
      grouped.map {
        case (_, iter) => (iter.head._1, iter.map(qa => qa._2))
      }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs: Iterable[(LangIndex, HighScore)] =>
      ...
      val mostCommon = groupedByLang.maxBy(_._2)
      val langLabel: String   = langs(mostCommon._1 / langSpread) // most common language in the cluster
      val langPercent: Double = mostCommon._2.size.toDouble / vs.size.toDouble * 100 // percent of the questions in the most common language
      val clusterSize: Int    = vs.size
      val medianScore: Int    = {
        if (clusterSize == 0) 0
        else medianOf(vs.toArray.sortBy(_._2))
      }
